...
Sight sight(30, 150, 150, RGB(255, 0, 0));
...
LRESULT _stdcall WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)		// оконная процедура принимает и обрабатывает все сообщения, отправленные окну
{
	...
	case WM_LBUTTONDOWN:
	{
		...
		if (sight.InnerPoint(x, y)) {
			sight.StartDragging(x, y);
		}
		return 0;
	}
	case WM_MOUSEMOVE:
	...
	{
		if (sight.IsResizing()) {
			sight.resize(GET_WHEEL_DELTA_WPARAM(wParam) / 30);
			InvalidateRect(hWnd, nullptr, false);
		}
		return 0;
	}
